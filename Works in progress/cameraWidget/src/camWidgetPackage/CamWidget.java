...
public class CamWidget extends WPICameraExtension {
    ...
    public static final String NAME = "Team 2974 Laptop Camera Widget";
    ...
    public WPIImage processImage(WPIColorImage rawImage) {
        //Color Processing?
        WPIBinaryImage greenBin = rawImage.getGreenChannel().getThreshold(200);
        ...
        WPIBinaryImage finalBin = blueBin.getAnd(redBin).getAnd(greenBin).getInverse();
...
//between .365 and 1.115
        ...
        finalBin.erode(2);
        //finalBin.dilate(6);//not sure what this 
        finalBin.dilate(2);
        ...
        for (WPIPolygon p : possiblePolygons) {
            ...
            for (WPIPolygon q : possiblePolygons) {
                ...
                rawImage.drawPoint(new WPIPoint(pCenterX, pCenterY), targetColor, 5);//what does 5 mean?
                ...
                if (Math.abs(pCenterX - qCenterX) < 20
                    ...
                    System.out.println(squareArea);   //prints something
                    break;
                }
            }
        }
        table.putDouble("x", contours.length);
        if (square != null) {
