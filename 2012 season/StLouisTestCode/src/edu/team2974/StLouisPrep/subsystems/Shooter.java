...
import edu.team2974.StLouisPrep.commands.DummySetShooterWheels;
...
public class Shooter extends Subsystem {
    ...
    public void zeroShooterJaguars() {
        try {
            jagTop.setX(0);
            jagBot.setX(0);
            RobotLogger.getInstance().logMessage("Zeroed Shooter Wheels");
        } catch (CANTimeoutException ex) {
    ...
    public void dummySetShooterWheels(double topWheel, double botWheel) {
        try {
            ...
            RobotLogger.getInstance().logMessage("Shooter wheels set to: top "+topWheel+" bot "+ botWheel);
        } catch (CANTimeoutException ex) {
            ex.printStackTrace();
        }
    }
