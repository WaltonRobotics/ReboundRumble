...
public class AutoShot extends CommandBase {
    ...
    protected void execute() {
        ...
        try {
            ...
            if(OI.targetTable.getBoolean("Target Found"))
            {
                ...
                RobotLogger.getInstance().logMessage("Target Found, setting shooter speed using equation");
                /*
            ...
            RobotLogger.getInstance().logMessage("No Key Defined in AutoShot");
    ...
    protected void end() {
        if(!targetFound)
        {
            RobotLogger.getInstance().logMessage("Reverting to Default Speed in Auto Shot");
