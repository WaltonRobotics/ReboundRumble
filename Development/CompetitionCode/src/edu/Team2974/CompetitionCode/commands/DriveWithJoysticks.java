...
import edu.Team2974.CompetitionCode.SpeedPair;
import edu.Team2974.CompetitionCode.SpeedQuadruple;
import edu.Team2974.CompetitionCode.SpeedSet;

public class DriveWithJoysticks extends CommandBase {
    private boolean reducedSpeed;
    public DriveWithJoysticks(boolean ds) {
        reducedSpeed = ds;
        requires(drive);
    }
    ...
    protected void execute() {
        ...
        if(reducedSpeed)
        {
            instructions = getReducedInstructions();
        }
        else
        {
    ...
    public double[] getReducedInstructions() {

        double forward = (oi.getLeftY() + oi.getRightY()) / 4;
        double turn = (oi.getLeftY() - oi.getRightY()) / 4;
        double sideways = -(oi.getLeftX() + oi.getRightX()) / 4;
